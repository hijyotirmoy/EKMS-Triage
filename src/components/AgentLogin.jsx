...
export function AgentLogin({ onLoginSuccess }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-70px)] items-center justify-center p-3.5 sm:p-6">
      <div className="w-full max-w-md rounded-2xl sm:rounded-3xl border border-slate-300 bg-white p-5 sm:p-7 shadow-xl shadow-slate-200/50">
        {/* Header */}
        <div className="mb-5 sm:mb-6 text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-500/30">
        ...
        {errorMsg && (
          <div className="mb-4 flex items-center gap-2 rounded-xl border border-rose-300 bg-rose-50 p-3 text-xs font-bold text-rose-800">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-300 bg-slate-50/80 py-3 sm:py-2.5 pl-10 pr-4 text-base sm:text-sm font-bold text-slate-950 transition focus:border-emerald-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-emerald-600 shadow-2xs"
              >
                {VALID_AGENTS.map((a) => (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-slate-50/80 py-3 sm:py-2.5 pl-10 pr-4 text-base sm:text-sm font-semibold text-slate-950 placeholder:text-slate-400 transition focus:border-emerald-600 focus:bg-white focus:outline-none focus:ring-1 focus:ring-emerald-600 shadow-2xs"
              />
            </div>
          </div>
      ...
      {conflictModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative z-10 w-full max-w-sm rounded-2xl border border-amber-300 bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-amber-50 text-amber-700 border border-amber-200">
              <AlertTriangle className="h-6 w-6" />
            </div>
            <h3 className="text-center text-base font-extrabold text-slate-950">
              Active Session Detected
            </h3>
            <p className="mt-2 text-center text-xs leading-relaxed text-slate-700">
              <strong className="text-emerald-700">{conflictedAgent}</strong> is
            ...
            <div className="mt-6 flex items-center gap-2.5">
              <button
                type="button"
                onClick={() => setConflictModalOpen(false)}
                className="flex-1 rounded-xl border border-slate-300 py-2.5 text-xs font-bold text-slate-700 transition hover:bg-slate-100"
              >
                Cancel
              </button>
              <button
